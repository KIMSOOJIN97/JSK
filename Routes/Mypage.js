    ...
    if(req.session.user){
        ...
        pool.getConnection(function(err, connection){
            ...
            connection.query(strsql, [req.session.user.id], function(err, rows1){
                ...
                var dep = "SELECT * FROM Department WHERE D_no in (SELECT D_no FROM Student where student_ID = ?)"
                connection.query(dep, [req.session.user.id], function(err, rows2){

                    res.render('Mypage', {rows1:rows1,userid: req.session.user.id,rows2:rows2});
                    connection.release();
                })
            })
        });  
...
router.post('/', function(req, res){
    ...
    if(req.session.user){
        ...
        pool.getConnection(function(err, connection){
            ...
            if(password != ""){
                var sql1 = "update student set password=? where student_ID=?";
                connection.query(sql1,[password,req.session.user.id],function(err,fields){
                    if(err) console.error(err);
                })
            }
            if(email != ""){
                var sql1 = "update student set email=? where student_ID=?";
                connection.query(sql1,[email,req.session.user.id],function(err,fields){
                    if(err) console.error(err);
                })
            }
            if(address != ""){
                var sql1 = "update student set address=? where student_ID=?";
            ...
            if(phone_number != ""){
                ...
                var sql1 = "update student set phone_number=? where student_ID=?";
