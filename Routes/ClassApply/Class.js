...
router.get('/', function(req, res){
    ...
    if(req.session.user){
        ...
        pool.getConnection(function(err, connection){
            ...
            var strsql = "SELECT * FROM opening_subject WHERE O_no IN (SELECT O_no FROM course_registration WHERE student_ID = ?) ORDER BY O_no;";
            connection.query(strsql, [req.session.user.id], function(err, rows1){
                if(err) console.error(err);
                
                strsql = "SELECT * FROM subject WHERE S_no IN (SELECT S_no FROM opening_subject WHERE O_no IN (SELECT O_no FROM course_registration WHERE student_ID = ?)) ORDER BY S_no";
